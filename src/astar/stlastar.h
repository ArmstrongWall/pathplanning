...
#pragma warning( disable : 4786 )//在WIN下请加入这条语句，屏蔽4786警告 ＃pragma warning (disable:4786) )
#endif
...
public: // data
	...
	// the f value of two nodes比较启发值的大小 STL heap algorithm parameter
...
public: // methods
	...
	// Set Start and goal states设置目标和起点
	void SetStartAndGoalStates( UserState &Start, UserState &Goal )
	{
		m_CancelRequest = false;

		m_Start = AllocateNode();//开始和结束节点
		m_Goal = AllocateNode();

		assert((m_Start != NULL && m_Goal != NULL));
		
		m_Start->m_UserState = Start;//设置起点
		m_Goal->m_UserState = Goal;//设置终点
